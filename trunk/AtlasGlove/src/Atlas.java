...
public class Atlas extends PApplet {/**
...
public void draw() {
  ...
  switch(playMode) {
    case 0:
    	...
    	break;
    case 1:
    	println("practice mode 1");
    	image(practice_mode, 5, 5); 
    	break;
    	
    case 2:
    	println("control mode 2");
...
public void capture() {
  ...
  for (int row = 0; row < video.height; row++) { // For each row
    for(int col = 0; col < video.width; col++) { // For each column
      ...
      int pixel = video.pixels[row * video.width + col];
...
class Target {
  ...
  public void include(int _x, int _y) {
    if (blob == null) {
  ...
  public boolean isSimilar(int thisPixel) {
  ...
  public void drawIt() {
    ...
    if (blob != null) {
      ...
      if (playMode == 0 && blob.width > panW &&
          blob.y + blob.height > videoH * 0.9f) {
        playMode = 1;
        demoAlpha = 0;
      }
      ...
      if (playMode < 2 && blob.width > panW &&
          blob.y + blob.height < videoH * 0.1f) {
        playMode = 2;
        press_alt_tab();
        simulateClick = true;
        simulateMove = true;
...
public void press_alt_tab() {
    robot.keyPress(18);
    robot.keyPress(9);
    robot.keyRelease(9);
    robot.keyRelease(18);
}
static public void main(String args[]) {   PApplet.main(new String[] { "Atlas" });}}